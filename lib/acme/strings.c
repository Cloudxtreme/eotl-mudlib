/*
    ...
    DESCRIPTION
        ...
        searcha_any_str()  - Like member() but matches any of several chars
        searcha_str_unescaped() - Like member() but ignores escaped chars
        ...
        aexplode()         - works the way the man page for explode says it
...
/*
    Description:
      Munges a directory name into a standard format.
    Parameters:
      name - An absolute filename that refers to a directory
    Returns:
      A directory name of the form /foo/bar/
*/
string
munge_dirname(string name)
{
  int len;
...
/*
    ...
    Returns:
      ...
      from <pos>.
    Notes:
      See member(), is_escaped(), searcha_str_unescaped()
*/
varargs int
searcha_any_str(string str, string elements, int pos, int step)
{
  ...
  for(size = strlen(str); (pos < size) && (pos >=0); pos += step)
    if((member(elements, str[pos]) != -1) && !is_escaped(str, pos))
      return pos;
...
/*
    ...
    Notes:
      See member(), is_escaped, searcha_any_str()
*/
varargs int
...
untabify( string str, int tabwidth )
{
    ...
    while( ( offset = member( str, '\t', offset ) ) > -1 )
...
private static closure lensort, varfilter;
                  ...
                  mixed func, mixed args, int kill_if_no_match )
{
    ...
    while( ( offset = member( str, varchar, offset ) ) > -1 &&
...
*/
private static closure indent_cl;
...
*/
private static closure center_cl;
...
/*--------------------------------- aexplode --------------------------------*/
/*
    description: Return an array of strings, created when the string str is
                 splitted into substrings as divided by del.
    parameters: str       - string to explode
                del       - delimeter to explode str with (default: " ");
    returns: string *    the array of substrings
    notes: aexplode() works the way the man page for explode says explode
           _should_ work (but doesn't)
*/
varargs string *
aexplode( string str, string del )
{
   string *foo;
   int     bar;
   ...
   // if the del is at the head of the string
   if ( strstr( str, del ) == 0 )
      foo = ({ "" }) + foo;

   // if the del is at the end of the string
   if ( strstr( str, del, ( bar = strlen( str ) - strlen( del ) ) ) == bar &&
       bar != -1 )
      foo += ({ "" });
